const taskReducer = (state = [], action) => {
    switch (action.type) {
      ...
      case 'UPDATE_TASK_TIME':
        return state.map((task)=>task._id === action.data.task_id ? {...task, total_time: action.data.time, time_start: 0}: task);
      case 'UPDATE_TASK_TIMER_STATUS':
        if(action.data.stop_all){
          return state.map((task)=>task._id === action.data.task_id ? {...task, time_start: 0}: {...task, time_start: 0});
        }else{
          return state.map((task)=>task._id === action.data.task_id ? {...task, time_start: 1}: {...task, time_start: 0});
        }
