                ...
                <Table responsive>
                  ...
                  {this.props.report && task_data_p.map((item)=>(
                      <tr key={item._id}>
                        ...
                        <td>{item.task}</td>
                        <td><div className="time_slot">{'1' == item.task_status && <span>{item.time}</span>}</div></td>
                  <td>{'1' == item.task_status && <span>{item.currency}{item.hourly}</span>}</td>
                        <td>{'1' == item.task_status && <span>{item.totalcost}</span>}</td>
                        <td><div className="time_slot">{'1' !== item.task_status && <span>{item.seq}</span>}</div></td>
                        <td>{item.created}</td>
                        <td>{item.completed}</td>
                        <td><div className="comment_break">{item.comment}</div></td>
                        <td>
                        {'1' == item.task_status && <span className="custombadge">{item.main_status == '3' && <Badge color="dark">Completed</Badge>}
                  ...
                  <td>
                    {'1' == item.task_status && <span className="custombadge">{item.main_status == '3' && <Button size="sm" className="btn-pill btn_action" color="danger" onClick={ () => this.handleReopen(item._idtask, item._id)} type="button" >Reopen</Button>}</span>}
                  </td>
                        <td><div style={ {background: item.background_color,
                          padding: "10px",
                          margin: "0 auto", 
                          width: "20px",
                          height: "20px"}}></div></td>
